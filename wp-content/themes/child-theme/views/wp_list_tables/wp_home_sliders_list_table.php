...
class wp_home_sliders_list_table extends WP_List_Table {
	protected $columns;
	protected $primary;
	protected $screen;
	...
	public function __construct() {
		...
		$data        = $map["wp_home_sliders"];
		self::$table = 'wp_home_sliders';
		...
		parent::__construct( [
			'singular' => apply_filters( "singular_wp_home_sliders", "wp_home_sliders item" ),
			'plural'   => apply_filters( "plural_wp_home_sliders", "wp_home_sliders items" ),
			'ajax'     => false
		] );
	}
	...
	public static function get_all( $per_page = 5, $page_number = 1 ) {
		global $wpdb;
		$sql = "SELECT * FROM wp_home_sliders";
	...
	public function no_items() {
		echo apply_filters( "no_items_wp_home_sliders", "No wp_home_sliders items found" );
	}

	function column_name( $item ) {
		$delete_nonce = wp_create_nonce( "delete_wp_home_sliders" );
		$title        = '<strong>' . $item['name'] . '</strong>';
		$actions      = [
			'delete' => sprintf( '<a href="?page=%s&action=%s&wp_home_sliders=%s&_wpnonce=%s">Delete</a>', esc_attr( $_REQUEST['page'] ), 'delete', absint( $item['ID'] ), $delete_nonce )
		];
	...
	public function prepare_items() {
		...
		$per_page     = $this->get_items_per_page( "wp_home_sliders_per_page", 5 );
	...
	public function process_bulk_action() {
		...
		if ( 'delete' === $this->current_action() ) {
			...
			if ( ! wp_verify_nonce( $nonce, "delete_wp_home_sliders" ) ) {
				die( 'Go get a life script kiddies' );
			} else {
				self::delete_one( absint( $_GET["wp_home_sliders"] ) );
